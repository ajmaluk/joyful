...
export function TopBar() {
  ...
  const isWorkspace = pathname.startsWith('/builder/') || pathname === '/builder'
  const isMarketingPage = marketingPaths.has(pathname) || (!isWorkspace && !pathname.startsWith('/dashboard'))
    ...
    return (
      <header
        className={\`fixed left-0 right-0 top-0 z-50 flex h-14 items-center justify-center px-4 transition-all duration-300 \${
          isScrolled
            ? 'border-b border-border/60 bg-background/82 shadow-[0_12px_36px_rgba(15,23,42,0.08)] backdrop-blur-xl supports-[backdrop-filter]:bg-background/62 dark:border-white/10 dark:shadow-[0_12px_36px_rgba(0,0,0,0.24)]'
...
fs.writeFileSync('components/joyful/top-bar.tsx', code);
