...
// .wrangler/tmp/pages-pFdFaq/functionsWorker-0.2340969396497099.mjs
...
__name2(getProviders, "getProviders");
var CORS_HEADERS = {
...
function shouldFallback(status, bodyText) {
  ...
  return lower.includes("rate limit") || lower.includes("overloaded") || lower.includes("capacity") || lower.includes("timeout") || lower.includes("temporarily unavailable") || lower.includes("too many requests") || lower.includes("service unavailable") || lower.includes("authorization failed") || lower.includes("invalid credentials") || lower.includes("api key") || lower.includes("unauthorized");
}
__name(shouldFallback, "shouldFallback");
__name2(shouldFallback, "shouldFallback");
...
async function onRequest(context) {
  ...
  if (providers.length === 0) {
    return new Response(
      JSON.stringify({
        error: {
          message: "No AI provider configured. Set NVIDIA_API_KEY (or VITE_NV_API_KEY) in Cloudflare Pages dashboard, or GROQ_API_KEY for fallback."
        }
      }),
  ...
  for (let i = 0; i < providers.length; i++) {
    ...
    const timeout = setTimeout(() => controller.abort(new DOMException("Provider request timed out", "TimeoutError")), 6e4);
    try {
...
// .wrangler/tmp/bundle-G1z7Ng/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-G1z7Ng/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.2340969396497099.js.map
