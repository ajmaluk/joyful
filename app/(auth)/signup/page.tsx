...
import { AuthShell, GithubMarker, GoogleMarker, ProviderButton } from '@/components/joyful/auth/AuthShell';
import { createAccountWithEmail, getFriendlyFirebaseAuthError, signInWithGithub, signInWithGoogle } from '@/lib/firebase';
...
export default function SignupPage() {
  ...
  const { authError: redirectAuthError, isAuthed, isAuthReady } = useAuth();
  ...
  useEffect(() => {
    ...
    const dest = pendingPrompt
      ? `/builder?prompt=${encodeURIComponent(pendingPrompt)}&mode=${pendingMode || 'build'}`
      : '/builder'
    router.replace(dest)
  }, [isAuthed, isAuthReady, pendingPrompt, pendingMode, router])
