...
export const ProjectsDashboard = memo(({ onSelectTemplate, onRunTemplate }: ProjectsDashboardProps) => {
  ...
  return (
    <section id="projects-dashboard" className="hidden md:block w-full max-w-[98%] mx-auto mb-0 mt-24 md:mt-32">
      <div className="rounded-t-[2.5rem] rounded-b-none p-6 md:p-8 pb-8 shadow-2xl bg-[var(--theme-bg-depth-1)] border-x border-t border-[var(--theme-border-default)]">
        {/* Header Tabs */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div className="flex items-center space-x-1 bg-[var(--theme-accent-bg)] p-1 rounded-2xl border border-[var(--theme-border-subtle)] w-full sm:w-auto animate-fade-in">
            <button
              onClick={() => setActiveTab('my-projects')}
              className={classNames(
                'flex-1 sm:flex-none px-4 md:px-5 py-2 text-xs md:text-sm font-medium rounded-xl transition-all duration-200',
                activeTab === 'my-projects' ? 'bg-[var(--theme-active-bg)] text-[var(--theme-text-primary)] shadow-sm' : 'bg-transparent text-[var(--theme-text-secondary)] hover:text-[var(--theme-text-primary)]'
              )}
            >
              My projects
            </button>
            <button
              ...
              className={classNames(
                'flex-1 sm:flex-none px-4 md:px-5 py-2 text-xs md:text-sm font-medium rounded-xl transition-all duration-200',
                activeTab === 'recent' ? 'bg-[var(--theme-active-bg)] text-[var(--theme-text-primary)] shadow-sm' : 'bg-transparent text-[var(--theme-text-secondary)] hover:text-[var(--theme-text-primary)]'
              )}
            >
              Recently viewed
            </button>
            <button
              ...
              className={classNames(
                'flex-1 sm:flex-none px-4 md:px-5 py-2 text-xs md:text-sm font-medium rounded-xl transition-all duration-200',
                activeTab === 'templates' ? 'bg-[var(--theme-active-bg)] text-[var(--theme-text-primary)] shadow-sm' : 'bg-transparent text-[var(--theme-text-secondary)] hover:text-[var(--theme-text-primary)]'
              )}
            >
              Lovable templates
            </button>
          ...
          {activeTab !== 'templates' && filteredProjects.length > 0 && (
            <a
              ...
              className="text-xs md:text-sm font-medium text-[var(--theme-text-secondary)] hover:text-[var(--theme-text-primary)] flex items-center group transition-colors ml-auto sm:ml-0"
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pb-2">
              {TEMPLATES.map((tpl) => (
                <div
                  ...
                  className="group cursor-pointer flex flex-col justify-between h-full rounded-2xl border border-[var(--theme-border-subtle)] bg-[var(--theme-bg-depth-2)]/40 p-5 hover:border-[var(--theme-border-strong)] hover:bg-[var(--theme-bg-depth-3)]/40 transition-all duration-300 relative overflow-hidden shadow-md"
                >
                  {/* Visual Header */}
                  <div>
                    <div className={classNames('w-12 h-12 rounded-xl bg-gradient-to-br flex items-center justify-center mb-4', tpl.gradient)}>
                      <div className={classNames('text-[var(--theme-text-primary)] text-xl', tpl.icon)} />
                    </div>
                    <h3 className="font-semibold text-[var(--theme-text-primary)] mb-2 group-hover:text-purple-300 transition-colors text-base">
                      {tpl.title}
                    </h3>
                    <p className="text-xs text-[var(--theme-text-secondary)] line-clamp-3 leading-relaxed mb-6">
                      {tpl.description}
                    </p>
                  </div>

                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-[var(--theme-border-subtle)]">
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-[var(--theme-active-bg)] text-[var(--theme-text-secondary)]">
                      {tpl.badge}
                    </span>
                    <button
                      onClick={(e) => {
                      ...
                      className="w-8 h-8 rounded-full bg-[var(--theme-active-bg)] hover:bg-white text-[var(--theme-text-secondary)] hover:text-black flex items-center justify-center transition-all duration-200"
            ...
            <>
              {filteredProjects.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  <div className="i-ph:folder-dashed text-4xl mb-4 text-[var(--theme-text-muted)]" />
                  <h3 className="text-[var(--theme-text-primary)] font-medium mb-1">No projects found</h3>
                  <p className="text-sm text-[var(--theme-text-muted)] max-w-xs">
                    {activeTab === 'recent'
                    ...
                    <div key={project.id} className="group cursor-pointer flex flex-col relative">
                      <div className="relative mb-4 aspect-video flex-shrink-0">
                        <a
                          ...
                          className="block w-full h-full rounded-2xl overflow-hidden border border-[var(--theme-border-subtle)] bg-[var(--theme-bg-depth-2)] group-hover:border-[var(--theme-border-default)] transition-all duration-300 shadow-md"
                        >
                          {project.thumbnail ? (
                            <img
                            ...
                            >
                              <span className="text-[var(--theme-text-primary)] text-3xl font-semibold opacity-70">
                        ...
                        <div className="absolute top-2 right-2 z-10">
                          <button
                            ...
                            className="w-8 h-8 rounded-full bg-black/60 hover:bg-black/80 text-[var(--theme-text-secondary)] hover:text-[var(--theme-text-primary)] transition-all border border-[var(--theme-border-subtle)] shadow-md flex items-center justify-center"
                          ...
                          {activeMenuId === project.id && (
                            <div className="absolute right-0 mt-1 w-32 bg-[var(--theme-bg-context-menu)] border border-[var(--theme-border-subtle)] rounded-xl py-1.5 shadow-xl z-50">
                              <a
                                href={`/chat/${project.urlId || project.id}`}
                                className="flex items-center gap-2 px-3 py-2 text-xs text-[var(--theme-text-secondary)] hover:bg-[var(--theme-hover-bg)] hover:text-[var(--theme-text-primary)] transition-colors"
                              >
                              ...
                              <button
                                ...
                                className="flex items-center gap-2 w-full text-left px-3 py-2 text-xs text-[var(--theme-text-secondary)] hover:bg-[var(--theme-hover-bg)] hover:text-[var(--theme-text-primary)] bg-transparent border-none transition-colors"
                              >
                      ...
                      <div className="flex items-start justify-between">
                        <a href={`/chat/${project.urlId || project.id}`} className="flex items-start space-x-3 flex-1 min-w-0">
                          <div
                            className={classNames(
                              'w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-semibold border border-white/20 text-[var(--theme-text-primary)] bg-gradient-to-tr',
                              getGradient(project.description || project.id)
                            )}
                          >
                            {getInitials(project.description || 'P')}
                          ...
                          <div className="min-w-0">
                            <h3 className="font-medium text-[var(--theme-text-primary)] mb-1 group-hover:underline truncate text-sm md:text-base">
                              {project.description || 'Untitled Project'}
                            </h3>
                            <p className="text-[11px] md:text-xs text-[var(--theme-text-muted)]">{formatTime(project.timestamp)}</p>
                          </div>
                        </a>
                      </div>
                    </div>
