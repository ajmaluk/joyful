...
export const generateFiles = ({ writer, modelId, isGroq }: Params) =>
  tool({
    description: isGroq ? 'Generate and write files to the sandbox filesystem.' : description,
    ...
    execute: async ({ sandboxId, paths }, { toolCallId, messages }) => {
      ...
      writer.write({
        id: toolCallId,
        type: 'data-generating-files',
        data: { sandboxId, paths: uploaded.map((file) => file.path), status: 'done' },
      })
